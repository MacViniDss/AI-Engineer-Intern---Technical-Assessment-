...
## Phase 2 — Worldwide (coming next)

To unlock worldwide support, open `app/utils/validators.py` and set:

```python
BRAZIL_ONLY: bool = False
```

Then remove the country guard in `app/routers/weather.py` (`_resolve_location`).
...
│   ├── models.py            # ORM: Location, WeatherQuery, WeatherRecord
...
│   │   └── maps.py          # Google Maps / Places wrapper
│   └── utils/
│       └── validators.py    # Location fuzzy match + input parsing
├── requirements.txt
├── .env.example
└── README.md
```
